...
for img_id, (t, p) in tqdm(p_trainval.items(), mininterval=20):
    ...
    if "clevr" in folder:
       if int(fold) > 0:
          df.astype(int).to_csv(os.path.join('../ProbKT/generate_data/clevr/clevr_all/train/',f"pseudo_label_{fold}",f"{img_id}.txt"), index = False)
       else:
          df.astype(int).to_csv(os.path.join('../ProbKT/generate_data/clevr/clevr_all/train/','pseudo_label',f"{img_id}.txt"), index = False)
    if "mol" in folder:
       if int(fold) > 0:
          df.astype(int).to_csv(os.path.join('../ProbKT/generate_data/molecules/molecules_all/train/',f"pseudo_label_{fold}",f"{img_id}.txt"), index = False)
       else:
