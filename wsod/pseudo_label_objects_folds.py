...
folder, tag, it, th, fold = sys.argv[1:]
print ('pseudo label params:', folder, tag, it, th, fold)
...
for img_id, (t, p) in tqdm(p_trainval.items(), mininterval=20):
    ...
    if "clevr" in folder:
       if int(fold) > 0:
          df.astype(int).to_csv(os.path.join('../rcnn-deepproblog/generate_data/clevr/clevr_all/train/',f"pseudo_label_{fold}",f"{img_id}.txt"), index = False)
       else:
    ...
    if "mol" in folder:
       if int(fold) > 0:
          df.astype(int).to_csv(os.path.join('../rcnn-deepproblog/generate_data/molecules/molecules_all/train/',f"pseudo_label_{fold}",f"{img_id}.txt"), index = False)
       else:
